...
public class GreaterOrEqualTo extends AbstractComparison {
    @Override
    ...
    protected boolean compareToString(Column<String> column, String literal) {
        for (int i = 0; i < column.size(); i++) {
            if (column.isNaN(i)) {
                continue;
            } else if (column.isNOVALUE(i) || column.get(i).compareTo(literal) < 0) {
                return false;
            }
        }
    ...
    protected boolean compareNumericColumns(Column c1, Column c2) {
        for (int i = 0; i < c1.size(); i++) {
            if (c1.isNaN(i) && c2.isNaN(i)) {
                continue;
            } else if (c1.isNOVALUE(i) || c2.isNOVALUE(i) || c2.isNaN(i)) {
                return false;
            } else if (c1.isNaN(i)) {
                continue;
            }
